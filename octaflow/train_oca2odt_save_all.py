...
parser.add_argument('--model_name',                type=str,   help='model name', default='octaflow')
...
# import oca2odt dataloader.
if args.dataset == 'oca2odt':
    from dataloaders.dataloader_oca2odt import OCA2ODTDataLoader
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # model
    model = OCTAFlowNet(version=args.encoder, inv_depth=False, max_depth=args.max_depth, pretrained=args.pretrain)
    # model = NewCRFDepth(version=args.encoder, inv_depth=False, min_depth=1, max_depth=args.max_depth, pretrained=args.pretrain)
    model.train()
    ...
    if args.dataset == 'oca2odt':
        dataloader = OCA2ODTDataLoader(args, 'train')
        dataloader_eval = OCA2ODTDataLoader(args, 'online_eval')
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                ...
                if eval_measures is not None:
                    ...
                    model_save_name_regular = '/model-{}'.format(global_step)
                    torch.save(checkpoint, args.log_directory + '/' + args.model_name + model_save_name_regular)
...
def main():
    ...
    exp_name = '%s'%(datetime.now().strftime('%Y%m%d_%H%M%S'))
    ...
    if save_files:
        ...
        command = 'cp octaflow/train.py ' + aux_out_path
        os.system(command)
        command = 'mkdir -p ' + networks_savepath + ' && cp octaflow/networks/*.py ' + networks_savepath
        os.system(command)
        command = 'mkdir -p ' + dataloaders_savepath + ' && cp octaflow/dataloaders/*.py ' + dataloaders_savepath
        os.system(command)
